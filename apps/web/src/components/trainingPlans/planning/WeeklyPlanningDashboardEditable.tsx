/**
 * WeeklyPlanningDashboardEditable.tsx — Dashboard editable de planning semanal
 *
 * Contexto:
 * - Dashboard completo para editar planning semanal del plan de entrenamiento
 * - Muestra distribución de cualidades físicas, carga de entrenamiento, progresión diaria
 * - Basado en diseño Figma "Weekly Planning" pero con capacidades de edición
 * - Permite editar distribución y carga de entrenamiento
 *
 * Responsabilidades:
 * - Cargar datos semanales con useGetTrainingPlanWeeklyPlanningQuery
 * - Renderizar PhysicalQualitiesRadarChart y PhysicalQualitiesList (editable)
 * - Renderizar TrainingLoadSliders (editable)
 * - Renderizar ProgressionChart (weekly - daily progression)
...
export interface WeeklyPlanningDashboardEditableProps {
    planId: number;
    weekStart?: string; // ISO date string (YYYY-MM-DD), default: current week start
}
...
export const WeeklyPlanningDashboardEditable: React.FC<
    WeeklyPlanningDashboardEditableProps
> = ({ planId, weekStart }) => {
    ...
    useEffect(() => {
        if (data) {
            ...
            // Seleccionar todos los cycle_ids de los microcycles por defecto
        ...
        return (
            <div className="flex flex-col items-center justify-center min-h-[400px] text-center">
                ...
                <h3 className="text-xl font-bold text-gray-900 mb-2">
                    No hay datos de Planning Semanal
                </h3>
                <p className="text-slate-500 max-w-md">
                    No se encontraron datos de planning semanal para este plan en la semana del {formatWeekRange(currentWeekStart)}.
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-8">
            ...
            {/* Progression Chart - Daily Progression */}
            <ChartCard title="Progresión Diaria">
                {data.daily_progression && data.daily_progression.length > 0 ? (
                    <ProgressionChart
                        data={data.daily_progression as unknown as Array<{
                            day: number;
                            date: string;
                    ...
                    <div className="flex items-center justify-center h-[300px] text-slate-500">
                        No hay datos de progresión diaria disponibles
                    </div>
                )}
            </ChartCard>
            ...
            <div className="bg-white rounded-lg shadow p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-gray-900 font-medium mt-1">
                            {data.cycles?.length || 0} microciclos
                        </p>
                    </div>
                </div>
            </div>
