/**
 * Toast.tsx — Componente de notificación toast
 *
 * Contexto:
 * - Notificaciones no intrusivas que aparecen en esquina superior derecha
 * - Auto-desaparecen después de un tiempo configurable
 * - Soporta variantes: success, error, warning, info
 * - Animaciones de entrada/salida suaves
 * - Diseño consistente con Alert pero optimizado para toasts
 *
 * @author Frontend Team
 * @since v6.1.0
 */

import React, { useEffect, useState, useCallback } from "react";

export type ToastVariant = "success" | "error" | "warning" | "info";

export interface ToastProps {
    id: string;
    variant: ToastVariant;
    message: string;
    duration?: number; // en milisegundos, default 4000
    onClose: (id: string) => void;
}

const variantStyles = {
    success: {
        container: "bg-green-50 border-green-200",
        text: "text-green-800",
        icon: "text-green-600",
    },
    error: {
        container: "bg-red-50 border-red-200",
        text: "text-red-800",
        icon: "text-red-600",
    },
    warning: {
        container: "bg-yellow-50 border-yellow-200",
        text: "text-yellow-800",
        icon: "text-yellow-600",
    },
    info: {
        container: "bg-blue-50 border-blue-200",
        text: "text-blue-800",
        icon: "text-blue-600",
    },
};

const icons = {
    success: (
        <svg className="w-5 h-5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
            <path
                fillRule="evenodd"
                d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
                clipRule="evenodd"
            />
        </svg>
    ),
    error: (
        <svg className="w-5 h-5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
            <path
                fillRule="evenodd"
                d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z"
                clipRule="evenodd"
            />
        </svg>
    ),
    warning: (
        <svg className="w-5 h-5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
            <path
                fillRule="evenodd"
                d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z"
                clipRule="evenodd"
            />
        </svg>
    ),
    info: (
        <svg className="w-5 h-5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
            <path
                fillRule="evenodd"
                d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z"
                clipRule="evenodd"
            />
        </svg>
    ),
};

export const Toast: React.FC<ToastProps> = ({
    id,
    variant,
    message,
    duration = 4000,
    onClose,
}) => {
    const [isVisible, setIsVisible] = useState(false);
    const [isExiting, setIsExiting] = useState(false);

    const styles = variantStyles[variant];
    const icon = icons[variant];

    const handleClose = useCallback(() => {
        setIsExiting(true);
        // Esperar a que termine la animación antes de llamar onClose
        setTimeout(() => {
            onClose(id);
        }, 300); // Duración de la animación de salida
    }, [id, onClose]);

    // Animación de entrada
    useEffect(() => {
        // Pequeño delay para activar animación
        const timer = setTimeout(() => setIsVisible(true), 10);
        return () => clearTimeout(timer);
    }, []);

    // Auto-cerrar después de duration
    useEffect(() => {
        const timer = setTimeout(() => {
            handleClose();
        }, duration);

        return () => clearTimeout(timer);
    }, [duration, handleClose]);

    return (
        <div
            className={`
                flex items-start gap-3 p-4 border rounded-lg shadow-lg min-w-[300px] max-w-[500px]
                transition-all duration-300 ease-in-out
                ${styles.container}
                ${isVisible && !isExiting ? "opacity-100 translate-x-0" : "opacity-0 translate-x-full"}
                ${isExiting ? "opacity-0 translate-x-full" : ""}
            `}
            role="alert"
        >
            <div className={styles.icon}>{icon}</div>
            <div className={`flex-1 ${styles.text} text-sm font-medium`}>{message}</div>
            <button
                type="button"
                onClick={handleClose}
                className={`${styles.icon} hover:opacity-70 transition-opacity`}
                aria-label="Cerrar notificación"
            >
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                    <path
                        fillRule="evenodd"
                        d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
                        clipRule="evenodd"
                    />
                </svg>
            </button>
        </div>
    );
};

